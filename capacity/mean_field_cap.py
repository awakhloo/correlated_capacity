'''we just modify the code so that the regression/projection isn't carried out. This is just the meanfield estimate.''' 
...
def manifold_analysis_corr(XtotT, kappa, n_t, t_vecs=None):
    ...
    for i in range(num_manifolds):
        ...
        if t_vecs is not None:
            a_Mfull, R_M, D_M, normv2 = each_manifold_analysis_D1(sD1, kappa, n_t, t_vec=t_vecs[i])
        else:
            a_Mfull, R_M, D_M, normv2 = each_manifold_analysis_D1(sD1, kappa, n_t)

        # Store the results
        a_Mfull_vec[i] = a_Mfull
        R_M_vec[i] = R_M
        D_M_vec[i] = D_M
        normv2s.append(normv2)
    return a_Mfull_vec, R_M_vec, D_M_vec, 1/np.mean(normv2s)#, res_/coeff0, KK
...
#                 print(s_f[-1, :])
    ...
    return a_Mfull, R_M, D_M, f_all.mean()
